...
public final class MiniJson {
 ...
 public static String stringify(Object v){ return stringify(v, 0); }
  ...
  public static String stringify(Object v, int cap){
    if(v==null)return"null";
    if(v instanceof String)return quote((String)v);
    if(v instanceof Number||v instanceof Boolean)return String.valueOf(v);
    if(v instanceof Map){StringBuilder b=new StringBuilder(Math.max(16,cap)).append('{');boolean f=true;for(Object e0:((Map<?,?>)v).entrySet()){Map.Entry<?,?> e=(Map.Entry<?,?>)e0;if(!f)b.append(',');f=false;b.append(quote(String.valueOf(e.getKey()))).append(':').append(stringify(e.getValue(),0));}return b.append('}').toString();}
    if(v instanceof Iterable){StringBuilder b=new StringBuilder(Math.max(16,cap)).append('[');boolean f=true;for(Object x:(Iterable<?>)v){if(!f)b.append(',');f=false;b.append(stringify(x,0));}return b.append(']').toString();}
    return quote(String.valueOf(v));
  }
 private static String quote(String s){StringBuilder b=new StringBuilder("\"");for(int i=0;i<s.length();i++){char c=s.charAt(i);switch(c){case '"':b.append("\\\"");break;case '\\':b.append("\\\\");break;case '\b':b.append("\\b");break;case '\f':b.append("\\f");break;case '\n':b.append("\\n");break;case '\r':b.append("\\r");break;case '\t':b.append("\\t");break;default:if(c<0x20){String h=Integer.toHexString(c);b.append("\\u");for(int z=h.length();z<4;z++)b.append('0');b.append(h);}else b.append(c);}}return b.append('"').toString();}
